...
int main (int argc, char **argv)
{
	...
	unsigned int dingoo_clock = DINGOO_DEFAULT_MHZ;
	unsigned int dingoo_brightness = DINGOO_DEFAULT_BRIGHT;
	...
	for (i = 1;i < argc;i++) /* V.V_121997 */
	{
		...
		if (strcasecmp(argv[i],"-brightness") == 0)
			dingoo_brightness=atoi(argv[i+1]);
		if ((strcasecmp(argv[i],"-clock") == 0) && (i<argc-1))
			dingoo_clock=atoi(argv[i+1]);
	...
	/* Dingoo Initialization - Sound HZ, BPP, Stereo, Mhz, Screen */
	dingoo_init(44100,16,0, dingoo_clock, dingoo_brightness);
