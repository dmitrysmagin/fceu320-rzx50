...
void AutoFire(void) {
	...
	if (FCEUMOV_Mode(MOVIEMODE_RECORD | MOVIEMODE_PLAY)) {
		rapidAlternator = AutoFirePattern[(AutoFireOffset + FCEUMOV_GetFrame())
				% AutoFirePatternLength];
	} else {
		rapidAlternator = AutoFirePattern[(AutoFireOffset + counter)
				% AutoFirePatternLength];
	}
}
...
void FCEUI_Autosave(void) {
	...
	if (AutosaveStatus[AutosaveIndex] == 1) {
		...
		if (AutosaveStatus[(AutosaveIndex + AutosaveQty - 1) % AutosaveQty]
				== 1) {
...
bool FCEU_IsValidUI(EFCEUI ui) {
	...
	case FCEUI_RESET:
		if (!GameInfo)
			return false;
		if (FCEUMOV_Mode(MOVIEMODE_TASEDIT | MOVIEMODE_PLAY))
			return false;
		break;
