	...
	while (fread(header, 1, 3, ips) == 3) {
		...
		if (!size) /* RLE */
		{
			...
			if ((offset + size) > (uint32) fp->size) {
				...
				if (!buf) {
					FCEU_printf(
							"  Oops.  IPS patch %d(type RLE) goes beyond end of file.  Could not allocate memory.\n",
							count);
					goto end;
				}
		...
		} else /* Normal patch */
		{
			...
			if ((offset + size) > (uint32) fp->size) {
				...
				if (!buf) {
					FCEU_printf(
							"  Oops.  IPS patch %d(type normal) goes beyond end of file.  Could not allocate memory.\n",
							count);
					goto end;
				}
				memset(buf + fp->size, 0, offset + size - fp->size);
...
void FCEU_SplitArchiveFilename(std::string src, std::string& archive,
		std::string& file, std::string& fileToOpen) {
				...
				if (strlen(tempu) >= 4) {
					...
					//if(!ext)
					{
						if (!strcasecmp(za, ".nes") || !strcasecmp(za, ".fds")
								|| !strcasecmp(za, ".nsf") || !strcasecmp(za,
								".unf") || !strcasecmp(za, ".nez"))
							break;
					}
...
FCEUFILE * FCEU_fopen(const char *path, const char *ipsfn, char *mode,
		char *ext, int index, const char** extensions) {
	FILE *ipsfile = 0;
	FCEUFILE *fceufp = 0;
	...
	if ((read && write) || (!read && !write)) {
		FCEU_PrintError(
				"invalid file open mode specified (only wb and rb are supported)");
		return 0;
	}
	...
	if (read) {
		...
		if (!ipsfile && !ipsfn)
			ipsfile = FCEUD_UTF8fopen(FCEU_MakeIpsFilename(DetermineFileBase(
					fceufp->logicalPath.c_str())), "rb");
