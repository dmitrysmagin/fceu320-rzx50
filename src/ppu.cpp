...
struct PPUREGS {
	...
	uint32 get_atread() {
		return 0x2000 | (v << 0xB) | (h << 0xA) | 0x3C0 | ((vt & 0x1C) << 1)
				| ((ht & 0x1C) >> 2);
	}
	...
	for (cc = 0; cc < 16; cc++) {
		for (xo = 0; xo < 8; xo++) {
			...
			for (pixel = 0; pixel < 8; pixel++) {
				...
				shiftr = (pixel + xo) / 8;
				shiftr *= 2;
				ppulut3[xo | (cc << 3)] |= ((cc >> shiftr) & 3) << (2 + pixel
						* 4);
			}
			//    printf("%08x\n",ppulut3[xo|(cc<<3)]);
		}
...
inline void FFCEUX_PPUWrite_Default(uint32 A, uint8 V) {
	...
	if (tmp < 0x2000) {
		...
		if (!(tmp & 3)) {
			if (!(tmp & 0xC))
				PALRAM[0x00] = PALRAM[0x04] = PALRAM[0x08] = PALRAM[0x0C] = V
						& 0x3F;
			else
...
void ppu_getScroll(int &xpos, int &ypos) {
	if (newppu) {
		...
		xpos = ((RefreshAddr & 0x400) >> 2) | ((RefreshAddr & 0x1F) << 3)
				| XOffset;

		ypos = ((RefreshAddr & 0x3E0) >> 2) | ((RefreshAddr & 0x7000) >> 12);
		if (RefreshAddr & 0x800)
			ypos += 240;
	}
}
//---------------
	...
	if (newppu) {
		if ((ppur.status.sl < 241) && PPUON) {
			...
			if (ppur.status.cycle < 64)
				...
				for (int i = spr_read.last; i != ppur.status.cycle; ++i) {
					if (i < 256) {
						...
						if (spr_read.found < (spr_read.ret + 1)) {
							...
							spr_read.ret
									= SPRAM[spr_read.found_pos[spr_read.ret | 3]];
					} else {
						if (!spr_read.found)
							spr_read.ret = SPRAM[252];
						else
...
static void CheckSpriteHit(int p) {
	...
	for (x = sphitx; x < (sphitx + 8) && x < l; x++) {
		...
		if ((sphitdata & (0x80 >> (x - sphitx))) && !(Plinef[x] & 64) && x
				< 255) {
			PPU_status |= 0x40;
...
#define TOFIXNUM (272-0x4)
		...
		if ((lastpixel - 16) >= 0) {
			InputScanlineHook(Plinef, spork ? sprlinebuf : 0, linestartts,
					lasttile * 8 - 16);
		}
		return;
	}
	...
	if (MMC5Hack && geniestage != 1) {
		if (MMC5HackCHRMode == 0 && (MMC5HackSPMode & 0x80)) {
			...
			for (X1 = firsttile; X1 < lasttile; X1++) {
				if ((tochange <= 0 && MMC5HackSPMode & 0x40) || (tochange > 0
						&& !(MMC5HackSPMode & 0x40))) {
#define PPUT_MMC5SP
#include "pputile.inc"
#undef PPUT_MMC5SP
...
#undef RefreshAddr
	...
	if ((lastpixel - 16) >= 0) {
		InputScanlineHook(Plinef, spork ? sprlinebuf : 0, linestartts, lasttile
				* 8 - 16);
	}
	Pline = P;
	firsttile = lasttile;
	...
	for (n = numsprites; n >= 0; n--, spr--) {
		...
		if (J) {
			if (n == 0 && SpriteBlurp && !(PPU_status & 0x40)) {
				...
				sphitdata = J;
				if (atr & H_FLIP)
					sphitdata = ((J << 7) & 0x80) | ((J << 5) & 0x40) | ((J
							<< 3) & 0x20) | ((J << 1) & 0x10) | ((J >> 1)
							& 0x08) | ((J >> 3) & 0x04) | ((J >> 5) & 0x02)
							| ((J >> 7) & 0x01);
			}
...
SFORMAT FCEUPPU_STATEINFO[] = { { NTARAM, 0x800, "NTAR" }, { PALRAM, 0x20,
		"PRAM" }, { SPRAM, 0x100, "SPRA" }, { PPU, 0x4, "PPUR" }, { &kook, 1,
		"KOOK" }, { &ppudead, 1, "DEAD" }, { &PPUSPL, 1, "PSPL" }, { &XOffset,
		1, "XOFF" }, { &vtoggle, 1, "VTOG" }, { &RefreshAddrT, 2
		| FCEUSTATE_RLSB, "RADD" }, { &TempAddrT, 2 | FCEUSTATE_RLSB, "TADD" },
		{ &VRAMBuffer, 1, "VBUF" }, { &PPUGenLatch, 1, "PGEN" }, { 0 } };

SFORMAT FCEU_NEWPPU_STATEINFO[] = { { &idleSynch, 1, "IDLS" }, { &spr_read.num,
		4 | FCEUSTATE_RLSB, "SR_0" }, { &spr_read.count, 4 | FCEUSTATE_RLSB,
		"SR_1" }, { &spr_read.fetch, 4 | FCEUSTATE_RLSB, "SR_2" }, {
		...
		&spr_read.found_pos[0], 4 | FCEUSTATE_RLSB, "SRx7" }, { &spr_read.ret,
		4 | FCEUSTATE_RLSB, "SR_4" }, { &spr_read.last, 4 | FCEUSTATE_RLSB,
		"SR_5" }, { &spr_read.mode, 4 | FCEUSTATE_RLSB, "SR_6" }, { &ppur.fv, 4
		| FCEUSTATE_RLSB, "PFVx" }, { &ppur.v, 4 | FCEUSTATE_RLSB, "PVxx" }, {
		&ppur.h, 4 | FCEUSTATE_RLSB, "PHxx" }, { &ppur.vt, 4 | FCEUSTATE_RLSB,
		"PVTx" }, { &ppur.ht, 4 | FCEUSTATE_RLSB, "PHTx" }, { &ppur._fv, 4
		| FCEUSTATE_RLSB, "P_FV" }, { &ppur._v, 4 | FCEUSTATE_RLSB, "P_Vx" }, {
		&ppur._h, 4 | FCEUSTATE_RLSB, "P_Hx" }, { &ppur._vt,
		4 | FCEUSTATE_RLSB, "P_VT" },
		{ &ppur._ht, 4 | FCEUSTATE_RLSB, "P_HT" }, { &ppur.fh, 4
				| FCEUSTATE_RLSB, "PFHx" }, { &ppur.s, 4 | FCEUSTATE_RLSB,
				"PSxx" }, { &ppur.status.sl, 4 | FCEUSTATE_RLSB, "PST0" }, {
				&ppur.status.cycle, 4 | FCEUSTATE_RLSB, "PST1" }, {
				&ppur.status.end_cycle, 4 | FCEUSTATE_RLSB, "PST2" }, { 0 } };
...
int FCEUX_PPU_Loop(int skip) {
...

	{
		...
		for (int sl = 0; sl < 241; sl++) {
			...
			if (sl != 0) {
				DEBUG(FCEUD_UpdatePPUView(scanline=yp,1)); DEBUG(FCEUD_UpdateNTView(scanline=yp,1));
			}
			...
			for (int xt = 0; xt < 32; xt++) {
				...
				if (sl != 0) {
					...
					const bool renderspritenow = SpriteON && rendersprites
							&& (xt > 0 || SpriteLeft8);
					const bool renderbgnow = ScreenON && renderbg && (xt > 0
							|| BGLeft8);
