...
int FCEU_InitVirtualVideo(void) {
	if (!XBuf) /* Some driver code may allocate XBuf externally. */
		...
		if (!(XBuf = (uint8*) (FCEU_malloc(256 * 256 + 16))) || !(XBackBuf
				= (uint8*) (FCEU_malloc(256 * 256 + 16)))) {
			return 0;
		}
...
void FCEU_PutImageDummy(void) {
	...
	if (guiMessage.howlong)
		guiMessage.howlong--; /* DrawMessage() */
}
#endif
...
int SaveSnapshot(void) {
	...
	for (u = lastu; u < 99999; u++) {
		pp = FCEUD_UTF8fopen((fn = strdup(
				FCEU_MakeFName(FCEUMKF_SNAP, u, "png").c_str())), "rb");
		if (pp == NULL)
			break;
		fclose(pp);
	}
...

	{
		uint8 pdata[256 * 3];
		for (x = 0; x < 256; x++)
			FCEUD_GetPalette(x, pdata + x * 3, pdata + x * 3 + 1, pdata + x * 3
					+ 2);
...

	{
		...
		if (!(mal = mork = dest = (uint8 *) malloc((totallines << 8)
				+ totallines)))
			goto PNGerr;
		//   mork=dest=XBuf;
		...
		if (compress(compmem, &compmemsize, mork, (totallines << 8)
				+ totallines) != Z_OK) {
			if (mal)
				free(mal);
			goto PNGerr;
...
void ShowFPS(void) {
	...
	if (showfps)
		sprintf(fpsmsg, "%2.0f", (double) booplimit / ((double) da
				/ FCEUD_GetTimeFreq()));
	currfps = (double) booplimit / ((double) da / FCEUD_GetTimeFreq());
	//puts((uint8 *)fpsmsg);
	if (showfps)
		DrawTextTrans(ClipSidesOffset + XBuf + (256 - 8 - 8 * 8)
				+ (FSettings.FirstSLine + 4) * 256, 256, (uint8 *) fpsmsg, 4);
