...
int InitVideo(FCEUGI *gi) {
	...
	FCEU_printf(" Video Mode: %d x %d x %d bpp %s\n", screen->w, screen->h,
			screen->format->BitsPerPixel, _fullscreen ? "full screen" : "");
	if (curbpp != 16 && curbpp != 24 && curbpp != 8 && curbpp != 32) {
		FCEU_printf(
				"  Sorry, %dbpp modes are not supported by FCE Ultra.  Supported bit depths are 8bpp, 16bpp, and 32bpp.\n",
				curbpp);
		KillVideo();
		return (0);
	}
	...
	IconSurface = SDL_CreateRGBSurfaceFrom((void *) fceu_playicon.pixel_data,
			32, 32, 24, 32 * 3, 0xFF, 0xFF00, 0xFF0000, 0x00);
#else
	...
	if (curbpp > 8)
		...
		if(!usingogl)
#endif
		InitBlitToHigh(curbpp >> 3, screen->format->Rmask,
				screen->format->Gmask, screen->format->Bmask, eefx, sponge);
#ifdef OPENGL
	if(usingogl)
...
void BlitScreen(uint8 *XBuf) {
	...
	if (curbpp > 8) {
		...
		if (BlitBuf)
			Blit8To8(XBuf + NOFFSET, dest, NWIDTH, tlines, TmpScreen->pitch, 1,
					1, 0, sponge);
		else
			Blit8To8(XBuf + NOFFSET, dest, NWIDTH, tlines, TmpScreen->pitch,
					exs, eys, eefx, sponge);
	}
	if (SDL_MUSTLOCK(TmpScreen))
